...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    let isAlreadyMadeOffer = findNft.auctionOffers.find(x => x.userId == req.currentUser?.id)
    if (!isAlreadyMadeOffer) throw new BadRequestError("You didn't made an offer on this NFT"); 
    ...
    try {
      ...
      res.json({
        status: true,
        data: updatedNft,
        msg: 'Auction offer cancelled successfully'
      });
    } catch (err) {
      console.log(err);
...
export { router as cancelAuctionOffer };