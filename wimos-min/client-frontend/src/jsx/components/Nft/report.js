...
export const ReportNFT = (props) => {
  // .store_domain
  // useEffect(() => {
  const storeDomain = useSelector(
    (state) => state.auth.selectedStore?.store_domain
  );

  // console.log("12------userr", storeDomain);

  // }, [props.getStore]);
  // const [nftId, SetNftID] = useState(props.getStore.store_domain);
  ...
  // const user = props.getUser;
  // console.log("nftId user", user);
  // useEffect(() => {
  //   console.log("nftId usssse", nftId);
  // }, [nftId]);

  //   useEffect(() => {
  //     dispatch(GetReports(user));
  //   }, []);
  //   useEffect(() => {
      ...
      Cell: (props) => {
        return (
          <div className="d-flex">
            ...
            >
              <a
                href={`https://${storeDomain}/product/${props.cell.row.original?.nftId}`}
                target="_blank"
              >
                {" "}
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h4 className="card-title">NFT reports</h4>
      </div>
